...
import fr.skoupi.stacks.StackHolder;
...
public abstract class EntityInsentient extends EntityLiving {
    ...
    private final StackHolder stackHolder; // SKOUPI
    ...
    public EntityInsentient(World world) {
        ...
        this.stackHolder = new StackHolder(); // SKOUPI
    }
    ...
    public void b(NBTTagCompound nbttagcompound) {
        ...
        stackHolder.saveStackAmountNBT(nbttagcompound); //SKOUPI
    ...
    public void a(NBTTagCompound nbttagcompound) {
        ...
        stackHolder.readStackAmount(nbttagcompound); //SKOUPI - Mob Stacker
    }
    ...
    public void die(DamageSource damagesource) {
        ...
        if (stackHolder.getStackAmount() > 1) {
            setCustomNameVisible(false);

            stackHolder.substractStackAmount(1);

            EntityInsentient spawning = (EntityInsentient) createNewEntityInstance(this.getClass(), world);
            spawning.stackHolder.setStackAmount(stackHolder.getStackAmount());
            spawning.setPosition(locX, locY, locZ);
            setStackName(spawning);

            world.addEntity(spawning, CreatureSpawnEvent.SpawnReason.STACK);
        } else if (stackHolder.getStackAmount() == 1) {
            setCustomNameVisible(false);
        }
    ...
    public void setStackName(EntityInsentient entityInsentient) {
        String entityName = getBukkitEntity().getType().name().toLowerCase().replace("_", "");
        String formarttedEntityName = entityName.substring(0, 1).toUpperCase() + entityName.substring(1);

        String stackName = "§6x%amount% §e%name%"
                .replace("%amount%", String.valueOf(getStackHolder().getStackAmount())).replace("%name%", formarttedEntityName);

        entityInsentient.setCustomName(stackName);
        entityInsentient.setCustomNameVisible(true);
    }

    private Entity createNewEntityInstance(Class<? extends Entity> tClass, World world) {
        if (tClass == null) {
            return null;
        }
        try {
            return tClass.getConstructor(new Class[]{World.class}).newInstance(world);
