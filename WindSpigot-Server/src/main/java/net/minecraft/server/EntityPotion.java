...
public class EntityPotion extends EntityProjectile {
		...
		if (!this.world.isClientSide)
		{
			...
			while (iterator.hasNext())
			{
				...
				if (d0 < 16.0D)
				{
					...
					if (entityliving == movingobjectposition.entity) d1 = 1.0D;
			...
			{ // do not process effects if there
				...
				for (LivingEntity victim : event.getAffectedEntities())
				{
					...
					if (this.getShooter() != null && entityliving instanceof EntityPlayer && !((EntityPlayer) entityliving).getBukkitEntity().canSee(this.getShooter().getBukkitEntity()))
						continue;
					...
					for (MobEffect mobeffect : list)
					{
						...
						} else
						{
							...
							if (j > 20) entityliving.addEffect(new MobEffect(i, j, mobeffect.getAmplifier()));
						}
					}
				}
			}
			...
			else this.world.triggerEffect(2002, new BlockPosition(this), this.getPotionValue());
			this.die();
		}
	...
	public void a (NBTTagCompound nbttagcompound)
	{
		...
		else this.setPotionValue(nbttagcompound.getInt("potionValue"));

		if (this.item == null) this.die();
	...
	public void b (NBTTagCompound nbttagcompound)
	{
		...
		if (this.item != null) nbttagcompound.set("Potion", this.item.save(new NBTTagCompound()));
