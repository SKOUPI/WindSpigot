	...
	public void track (List<EntityHuman> list)
	{
		...
		{ // Paper
			...
			{ // Paper - moved back up
				...
				for (EntityPlayer trackedPlayer : this.trackedPlayers)
				{
					...
					if (packet != null)
					{
						trackedPlayer.playerConnection.queuePacket(packet);
					}
				}
			}
		...
		if (this.tickCount % this.c == 0 || this.tracker.ai || this.tracker.getDataWatcher().a())
		{
			...
			if (this.tracker.vehicle == null)
			{
				...
				if (this.u)
				{
					...
					if (d4 > d3 * d3 || d4 > 0.0D && this.tracker.motX == 0.0D && this.tracker.motY == 0.0D && this.tracker.motZ == 0.0D)
					{
						...
						this.broadcastInternal(new PacketPlayOutEntityVelocity(this.tracker.getId(), this.motionX, this.motionY, this.motionZ));
					}
				}
			...
			} else
			{
				...
				if (flag2)
				{
					this.broadcastInternal(new PacketPlayOutEntity.PacketPlayOutEntityLook(this.tracker.getId(), (byte) i, (byte) j, this.tracker.onGround));
					this.yRot = i;
					this.xRot = j;
				}
		...
		if (entityplayer != this.tracker)
		{
			...
			if (this.c(entityplayer))
			{
				if (!isPlayerEntityTracked && (this.e(entityplayer) || this.tracker.attachedToPlayer))
				{
					...
					if (packet instanceof PacketPlayOutSpawnEntityExperienceOrb || packet instanceof PacketPlayOutSpawnEntity)
						entityplayer.playerConnection.sendPacket(packet);
					else entityplayer.playerConnection.queuePacket(packet);
					if (!this.tracker.getDataWatcher().d())
					{
						entityplayer.playerConnection.queuePacket(new PacketPlayOutEntityMetadata(this.tracker.getId(), this.tracker.getDataWatcher(), true));
					}

					if (this.tracker != null)
					{

						if (tracker.getNBTTag() != null)
						{
							entityplayer.playerConnection.queuePacket(new PacketPlayOutUpdateEntityNBT(this.tracker.getId(), tracker.getNBTTag()));
						}
						...
						if (this.tracker instanceof EntityLiving)
						{
							...
							{
								entityplayer.playerConnection.queuePacket(new PacketPlayOutUpdateAttributes(this.tracker.getId(), collection));
							}
						}
						...
						{
							entityplayer.playerConnection.queuePacket(new PacketPlayOutEntityVelocity(this.tracker.getId(), this.tracker.motX, this.tracker.motY, this.tracker.motZ));
						}

						if (this.tracker.vehicle != null)
						{
							entityplayer.playerConnection.queuePacket(new PacketPlayOutAttachEntity(0, this.tracker, this.tracker.vehicle));
						}

						if (this.tracker instanceof EntityInsentient && ((EntityInsentient) this.tracker).getLeashHolder() != null)
						{
							entityplayer.playerConnection.queuePacket(new PacketPlayOutAttachEntity(1, this.tracker, ((EntityInsentient) this.tracker).getLeashHolder()));
						}
						...
						if (this.tracker instanceof EntityLiving)
						{
							for (int i = 0; i < 5; ++i)
							{
								...
								if (itemstack != null)
								{
									entityplayer.playerConnection.queuePacket(new PacketPlayOutEntityEquipment(this.tracker.getId(), i, itemstack));
								}
							}
						}
						...
						if (this.tracker instanceof EntityHuman)
						{
							...
							{
								entityplayer.playerConnection.queuePacket(new PacketPlayOutBed(entityhuman, new BlockPosition(this.tracker)));
							}
						}
						...
						{ // SportPaper - avoid processing entities that can't
							...
							// lastHeadYaw));
							entityplayer.playerConnection.queuePacket(new PacketPlayOutEntityHeadRotation(this.tracker, (byte) lastHeadYaw));
							// SportPaper end
						}
						// CraftBukkit end
						...
						if (this.tracker instanceof EntityLiving)
						{
							...
							{
								entityplayer.playerConnection.queuePacket(new PacketPlayOutEntityEffect(this.tracker.getId(), mobeffect));
							}
						}
					}
				}
