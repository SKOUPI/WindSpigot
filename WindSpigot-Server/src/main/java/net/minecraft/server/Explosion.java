			...
			if (!entity.aW())
			{
				if (!entity.dead)
				{
					...
					if (distanceSquared <= 64.0D && distanceSquared != 0.0D)
					{
						...
						{
							this.getBlockDensityAsync(vec3d, entity.getBoundingBox()).thenAccept((d12) -> AsyncUtil.runPostTick(() -> {
								processEntityKnockback(entity, d7, finalD, finalD1, finalD11, f3, d12);
							}));
						} else
						{
							processEntityKnockback(entity, d7, finalD, finalD1, finalD11, f3, this.getBlockDensitySync(vec3d, entity.getBoundingBox()));
						}
						// WindSpigot end
					}
				}
			...
			for (; f > 0.0F; f -= 0.22500001F)
			{
				...
				if (position.getX() != floorX || position.getY() != floorY || position.getZ() != floorZ)
				{
					...
					if (block != Blocks.AIR)
					{
						...
						if (f > 0.0F && (this.source == null || this.source.a(this, this.world, position, iblockdata, f)) && position.getY() < 256 && position.getY() >= 0)
